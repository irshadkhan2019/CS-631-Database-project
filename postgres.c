 ...
 * ----------------
 */
//new function declararion
char* convert_query(const char *query);
...
int starts_with_keyword(const char *query, const char *keyword);
char* convert_query(const char *query);
void transformQuery(const char *inputQuery, char *finalQuery);
...
exec_simple_query(const char *query_string)
{
	...
	bool		success = false; // Add success tracking variable
	int         num_rows = 0;
	char		msec_str[32];
			...
			case PqMsg_Query:
				{
					...
					if (am_walsender)
					{
						if (!exec_replication_command(query_string))
						 ...
						 if (starts_with_keyword(query_string, "INSERT"))
						{
							char *modified_query = convert_query(query_string);
							printf("Modified query Insert: %s\n", modified_query);
							exec_simple_query(modified_query);
						}
						else if (starts_with_keyword(query_string, "SELECT"))
						{	printf("CACHED output:");
							iscached = exec_simple_query(query_string);
							printf("Boolean val : %s\n", iscached ? "true" : "false");
							if(!iscached){
								char finalQuery[768];
								char *modified_query = convert_query(query_string);
								printf("Modified query Select: %s\n", modified_query);
								//  Use FDW query
								printf("QUERY Not Cached ,caching and returning the result.:\n");
								transformQuery(modified_query, finalQuery);

								printf("Transformed Query:\n");
								printf("%s\n", finalQuery);

								exec_simple_query(finalQuery);
								exec_simple_query(query_string);
...
// char* convert_query(const char *query) {
// 	// Variables to hold positions and buffer size
// 	char *from_pos = NULL;
// 	char *table_pos = NULL;
// 	char *new_query;
// 	size_t query_len = strlen(query);
// 	size_t buffer_size = query_len + 50;  // Extra space for 'local_schema.'
//
// 	// Allocate memory for the new query
// 	new_query = (char *)malloc(buffer_size);
// 	if (new_query == NULL) {
// 		perror("Unable to allocate memory for new query");
// 		exit(EXIT_FAILURE);
// 	}
//
// 	char *current_pos = (char *)query;
//
// 	// Search for 'FROM' keyword (case-insensitive)
// 	while (*current_pos) {
// 		if (strncasecmp(current_pos, "from", 4) == 0) {
...
//
// 	if (from_pos != NULL) {
// 		// Copy the part of the query before 'FROM' (including the 'FROM')
// 		strncpy(new_query, query, from_pos - query + 4);  // Include 'FROM'
//
// 		// Skip past "FROM" and any spaces after it
// 		table_pos = from_pos + 4;
// 		while (*table_pos == ' ' || *table_pos == '\t') {
// 			table_pos++;
// 		}
//
// 		// Append 'local_schema.' before the table name and ensure a space after 'FROM'
// 		strcat(new_query, " local_schema.");
//
// 		// Append the rest of the query after the table name
// 		strcat(new_query, table_pos);
// 	} else {
// 		// If 'FROM' is not found, return the original query as is
...
int starts_with_keyword(const char *query, const char *keyword) {
    ...
    while (*query == ' ' || *query == '\t') {
        query++;  // Skip leading whitespace
    }
    return strncasecmp(query, keyword, keyword_len) == 0;
}

char* convert_query(const char *query) {
    char *new_query;
    size_t query_len = strlen(query);
    size_t buffer_size = query_len + 100;
    ...
    const char *current_pos = query;
    while (*current_pos == ' ' || *current_pos == '\t') {
        current_pos++;  // Skip leading whitespace
    }

    if (starts_with_keyword(current_pos, "SELECT")) {
        // Handle SELECT query
        const char *from_pos = strcasestr(current_pos, "FROM");
        if (from_pos) {
            size_t prefix_len = from_pos - query + 5;
            strncpy(new_query, query, prefix_len);
            new_query[prefix_len] = '\0';

            strcat(new_query, "local_schema.");

            // Append the rest of the query
            strcat(new_query, from_pos + 5);  // Skip "FROM "
        } else {

            strcpy(new_query, query);
        }
    } else if (starts_with_keyword(current_pos, "INSERT")) {
        // Handle INSERT query
        const char *into_pos = strcasestr(current_pos, "INTO");
        if (into_pos) {
            size_t prefix_len = into_pos - query + 5;
            strncpy(new_query, query, prefix_len);
            new_query[prefix_len] = '\0';
            strcat(new_query, "local_schema.");
            strcat(new_query, into_pos + 5);
        } else {
...
// Function to transform the query
void transformQuery(const char *inputQuery, char *finalQuery) {
	// Find the starting position of the table name
	const char *selectKeyword = "SELECT * FROM ";
	const char *tableStart = strcasestr_custom(inputQuery, selectKeyword);
	if (!tableStart) {
		printf("Invalid query format: Missing 'SELECT * FROM'.\n");
		return;
	}
	tableStart += strlen(selectKeyword);

	// Find the end of table schema and name (up to the first space or semicolon)
	const char *endOfTable = strchr(tableStart, ' ');
	if (!endOfTable) {
		endOfTable = strchr(tableStart, ';'); // Handle if no space but ends with a semicolon
	}
	if (!endOfTable) {
		printf("Invalid query format: Cannot determine table name.\n");
		return;
	}

	// Extract the schema and table name
	size_t tableNameLength = endOfTable - tableStart;
	char tableName[128] = {0};
	strncpy(tableName, tableStart, tableNameLength);

	// Create the final INSERT query
	snprintf(finalQuery, 768, "INSERT INTO public.%s %s",
			 strchr(tableName, '.') ? strchr(tableName, '.') + 1 : tableName,
			 inputQuery);
}
